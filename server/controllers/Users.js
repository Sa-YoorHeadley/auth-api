...
const PatchProfile = async (req, res) => {
  ...
  ValidateProfilePatch(_id, profileData)
    .then((profileData) => {
      UpdateUserById(_id, profileData)
        .then((user) => {
          return res
            .status(200)
...
const PatchPassword = async (req, res) => {
  ...
  const { _id } = req.params;
  ValidatePasswordPatch(_id, passwordData)
    .then((passwordData) => {
      UpdatePasswordById(_id, passwordData.newPassword)
        .then((user) => {
          return res
            .status(200)
